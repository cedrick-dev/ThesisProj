...
public class ChildSignedInActivity extends AppCompatActivity
		implements OnPermissionExplanationListener, OnPasswordValidationListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (childFirstLaunch)
			...
			// Listen for Nudity Filter changes from Web Dashboard
			DatabaseReference nudityFilterRef = FirebaseDatabase.getInstance()
					.getReference("users/childs/" + user.getUid() + "/contentFilters/nudity");
			...
			nudityFilterRef.addValueEventListener(new ValueEventListener() {
				...
				public void onDataChange(@NonNull DataSnapshot snapshot) {
					...
					if (isEnabled != null && isEnabled) {
						// Filter enabled, start service if not running
						if (!NsfwProtectionHelper.INSTANCE.isServiceRunning(ChildSignedInActivity.this)) {
							if (NsfwProtectionHelper.INSTANCE.hasRequiredPermissions(ChildSignedInActivity.this)) {
								NsfwProtectionHelper.INSTANCE.requestMediaProjection(ChildSignedInActivity.this);
							} else {
								NsfwProtectionHelper.INSTANCE.requestOverlayPermission(ChildSignedInActivity.this);
							}
						}
					} else {
						// Filter disabled, stop service
						NsfwProtectionHelper.INSTANCE.stopScreenFilterService(ChildSignedInActivity.this);
					}
				}
	...
	private void startPermissionExplanationDialogFragment() {
		...
		permissionExplanationDialogFragment.show(getSupportFragmentManager(),
				Constant.PERMISSION_EXPLANATION_FRAGMENT_TAG);
	}
	...
	private void startPasswordValidationDialogFragment() {
		...
		passwordValidationDialogFragment.show(getSupportFragmentManager(),
				Constant.PASSWORD_VALIDATION_DIALOG_FRAGMENT_TAG);
	}
